...
def convert_html_to_markdown(html: str) -> str | None:
    ...
    if not html or not html.strip():
        logging.info(
            "Received empty HTML for Markdown conversion, returning empty string."
        )
        return ""
...
def _fetch_linkedin_job_ids(search_query: str, location: str) -> list:
    ...
    while start <= max_start:
        ...
        while retries <= config.MAX_RETRIES:
            try:
                res = requests.get(
                    target_url, headers=headers, timeout=config.REQUEST_TIMEOUT
                )
                ...
                if e.response.status_code == 429 and retries < config.MAX_RETRIES:
                    ...
                    logging.warning(
                        f"Error 429: Too Many Requests. Retrying attempt {retries}/{config.MAX_RETRIES} after {wait_time:.2f} seconds..."
                    )
        ...
        if res is None:
            logging.error(
                f"Failed to fetch {target_url} after {retries} retries. Stopping pagination for this query."
            )
            break
        ...
        if not all_jobs_on_this_page:
            logging.info(
                f"No job listings ('li' elements) found on page at start={start}, stopping."
            )
            break

        logging.info(
            f"Found {len(all_jobs_on_this_page)} potential job elements on this page."
        )
        ...
        logging.info(
            f"Added {jobs_found_this_iteration} unique job IDs from this page."
        )

        if jobs_found_this_iteration == 0 and len(all_jobs_on_this_page) > 0:
            logging.info(
                "Found list items but no new job IDs extracted, potentially end of relevant results or parsing issue."
            )
            break
    ...
    logging.info(
        f"--- Finished Phase 1: Found {len(job_ids_list)} unique job IDs during scraping ---"
    )
    return job_ids_list
...
def _fetch_linkedin_job_details(job_id: str) -> dict | None:
    ...
    while retries <= config.MAX_RETRIES:
        try:
            resp = requests.get(
                job_detail_url, headers=headers, timeout=config.REQUEST_TIMEOUT
            )
            ...
            if e.response.status_code == 429 and retries < config.MAX_RETRIES:
                ...
                logging.warning(
                    f"Error 429 for job ID {job_id}. Retrying attempt {retries}/{config.MAX_RETRIES} after {wait_time:.2f} seconds..."
                )
                time.sleep(wait_time)
                user_agent = random.choice(user_agents.USER_AGENTS)
                headers = {"User-Agent": user_agent}
            ...
            logging.error(
                f"Request Exception fetching details for job ID {job_id}: {e}"
            )
            return None

    if resp is None:
        logging.error(
            f"Failed to fetch details for job ID {job_id} after {retries} retries (unexpected state)."
        )
        return None
    ...
    try:
        ...
        try:
            ...
            if location_span:
                ...
                if subtitle_div:
                    location_span_fallback = subtitle_div.find(
        ...
        if description_html.strip():
            ...
            logging.warning(
                f"Description HTML was empty for job ID {job_id}. Skipping conversion."
            )
        ...
        logging.error(
            f"General Error processing details for job ID {job_id} after successful fetch: {e}"
        )
        return None
...
def process_linkedin_query(search_query: str, location: str, limit: int = None) -> list:
    ...
    logging.info(
        f"Found {len(scraped_job_ids)} raw job IDs, {len(unique_linkedin_job_ids)} unique IDs after scraping."
    )
    ...
    logging.info(
        f"Identified {len(new_job_ids_to_process)} new job IDs to fetch details for."
    )
    ...
    if limit is not None and len(new_job_ids_to_process) > limit:
        logging.info(
            f"Truncating new_job_ids_to_process from {len(new_job_ids_to_process)} to {limit} to stay within source limit."
        )
        ...
        if details:
            ...
            if description and description.strip():
                if "job_id" in details and details["job_id"] is not None:
                    ...
                    logging.warning(
                        f"Fetched details for {job_id} but missing 'job_id' key. Skipping."
                    )
            else:
                logging.warning(
                    f"Skipping job ID {job_id} due to missing or empty description."
                )
        else:
            logging.warning(
                f"Skipping job ID {job_id} as detail fetching failed or returned no data."
            )

    logging.info(
        f"--- Finished Phase 2: Successfully fetched details for {processed_count} new job(s) ---"
    )
...
def _fetch_careers_future_jobs(search_query: str) -> list:
    ...
    careers_future_suggestions_api_url = (
        "https://api.mycareersfuture.gov.sg/v2/skills/suggestions"
    )
    ...
    try:
        logging.info(
            f"Fetching skill suggestions for query: '{search_query}' from {careers_future_suggestions_api_url}"
        )
        ...
        skillUuids = [
            skill_dict["uuid"] for skill_dict in skills_list if "uuid" in skill_dict
        ]
        logging.info(
            f"Successfully retrieved {len(skillUuids)} skill UUIDs for '{search_query}'."
        )
        if not skillUuids:
            logging.warning(
                f"No skill UUIDs found for query '{search_query}'. Job search will proceed without specific skill filtering."
            )
        ...
        response_text = (
            http_err.response.text if http_err.response is not None else "N/A"
        )
        logging.error(
            f"HTTP error during skill suggestions: {http_err} - Status: {status_code}"
        )
        logging.debug(
            f"Skill suggestions error response content: {response_text[:500]}"
        )
        return []
        ...
        content_for_log = (
            ...
            if "skills_suggestions_response" in locals() and skills_suggestions_response
            else "N/A"
        )
        logging.error(
            f"Could not decode JSON response for skill suggestions. Content: {content_for_log[:500]}"
        )
        return []
    ...
    try:
        while current_search_url:
            ...
            logging.info(
                f"Job search API call {total_api_calls_for_search}: POST to {current_search_url}"
            )
            ...
            logging.info(
                f"Retrieved {len(current_page_jobs)} job items from this page. Total items collected: {len(all_job_items)}."
            )

            # Log total results reported by API
            if "total" in search_results_data and total_api_calls_for_search == 1:
                logging.info(
                    f"API reports total potential jobs matching criteria: {search_results_data['total']}"
                )

            # Get the next page URL. The API provides a full URL.
            next_page_link_info = search_results_data.get("_links", {}).get("next", {})
            current_search_url = (
                next_page_link_info.get("href") if next_page_link_info else None
            )
        ...
        logging.info(
            f"Completed job search. Total API calls made for search: {total_api_calls_for_search}."
        )
        ...
        logging.error(
            f"HTTP error during job search: {http_err} - Status: {status_code}"
        )
        ...
        content_for_log = (
            ...
            if "search_response" in locals() and search_response
            else "N/A"
        )
        logging.error(
            f"Could not decode JSON response during job search. Content: {content_for_log[:500]}"
        )
    ...
    logging.info(
        f"Returning {len(all_job_items)} total job items for query '{search_query}'."
    )
...
def _fetch_careers_future_job_details(job_id: str) -> dict | None:
    ...
    logging.info(
        f"Attempting to fetch job details for ID: {job_id} from URL: {api_url}"
    )
    ...
    try:
        ...
        if raw_description_html.strip():
            ...
            logging.warning(
                f"Raw description was empty for Careers Future job ID {job_id}. Skipping conversion."
            )
        ...
        job_details = {
            ...
            "location": "Singapore",
            "level": job_data.get("positionLevels", [{"position": "Not applicable"}])[
                0
            ].get("position", "Not applicable"),
        ...
        if status_code == 404:
            logging.warning(
                f"Job details not found (404) for ID: {job_id} at {api_url}."
            )
        else:
            logging.error(
                f"HTTP error occurred while fetching job details for ID '{job_id}': {http_err} - Status: {status_code}"
            )
            logging.debug(f"Error response content: {response_text[:500]}")
    except requests.exceptions.ConnectionError as conn_err:
        logging.error(
            f"Connection error occurred while fetching job details for ID '{job_id}': {conn_err}"
        )
    except requests.exceptions.Timeout as timeout_err:
        logging.error(
            f"Timeout error occurred while fetching job details for ID '{job_id}': {timeout_err}"
        )
    except requests.exceptions.RequestException as req_err:
        logging.error(
            f"An error occurred during the request for job details for ID '{job_id}': {req_err}"
        )
    except json.JSONDecodeError:
        content_for_log = (
        ...
        logging.error(
            f"Failed to decode JSON response for job details for ID '{job_id}'. Content: {content_for_log[:500]}"
        )
...
def process_careers_future_query(search_query: str, limit: int = None) -> list:
    ...
    try:
        job_ids_set_supabase, company_title_set_supabase = (
            supabase_utils.get_existing_jobs_from_supabase()
        )
        logging.info(
            f"Phase 2: Supabase returned {len(job_ids_set_supabase)} existing IDs and {len(company_title_set_supabase)} company/title pairs."
        )
    except Exception as e:
        logging.error(f"Failed to fetch existing jobs from Supabase: {e}")
        logging.warning(
            "Proceeding without Supabase data; all fetched jobs will be considered new."
        )
    ...
    for job_item in careers_future_jobs:
        if not isinstance(job_item, dict):
            logging.warning(
                f"Skipping invalid job item (not a dict): {str(job_item)[:100]}"
            )
            continue
        ...
        if job_uuid and job_uuid in job_ids_set_supabase:
            logging.debug(
                f"Skipping job (ID exists in Supabase): UUID='{job_uuid}', Title='{job_item.get('title', 'N/A')}'"
            )
        ...
        if normalized_company and normalized_title:
            ...
            if company_title_key in company_title_set_supabase:
                logging.debug(
                    f"Skipping job (Company/Title combo exists in Supabase): UUID='{job_uuid}', Company='{normalized_company}', Title='{normalized_title}'"
                )
                skipped_by_combo_count += 1
                continue
        elif job_uuid:
            logging.debug(
                f"Job UUID='{job_uuid}' has no company/title for combo check. Will be added if ID is new."
            )
        else:
            logging.warning(
                f"Job item has no UUID and insufficient company/title for matching: {str(job_item)[:100]}"
            )
    ...
    for job_id in new_job_ids_to_process:
        ...
        if details:
            ...
            ):  # Ensure it's not None or an empty/whitespace string
                if "job_id" in details and details["job_id"] is not None:
                    detailed_new_jobs.append(details)
                    processed_count += 1
                else:
    ...
    logging.info(
        f"--- Finished Phase 4: Successfully fetched details for {processed_count} new job(s) ---"
    )
    return detailed_new_jobs
...
if __name__ == "__main__":
    ...
    if "linkedin" in config.SCRAPING_SOURCES:
        ...
        for query in config.LINKEDIN_SEARCH_QUERIES:
            ...
            new_linkedin_job_details = process_linkedin_query(
                query, config.LINKEDIN_LOCATION, limit=max_jobs_per_search
            )
    ...
    if "careers_future" in config.SCRAPING_SOURCES:
        ...
        for query in config.CAREERS_FUTURE_SEARCH_QUERIES:
            ...
            # 1. Process the query: Scrape IDs, filter, fetch new details
            new_careers_future_job_details = process_careers_future_query(
                query, limit=max_jobs_per_search
            )
