...
async def _check_single_linkedin_job_active(
    job_id: str, client: httpx.AsyncClient
) -> bool | None:
    """
    ...
    while retries <= config.ACTIVE_CHECK_MAX_RETRIES:
        try:
            ...
            logging.debug(
                f"Checking job {job_id} (Attempt {retries + 1}/{config.ACTIVE_CHECK_MAX_RETRIES + 1}) URL: {job_detail_url} with UA: {user_agent}"
            )
            ...
            if response.status_code >= 400:
                logging.warning(
                    f"Job {job_id} check failed with status {response.status_code}. Assuming active for now."
                )
            ...
            for keyword in inactive_keywords:
                if keyword in response_text_lower:
                    logging.info(
                        f"Job {job_id} contains inactive keyword '{keyword}'. Marking as inactive."
                    )
                    return True

            # If status is OK and no inactive keywords found
            logging.debug(
                f"Job {job_id} appears active (Status: {response.status_code})."
            )
            return False
            ...
            logging.warning(
                f"Request error checking job {job_id} (Attempt {retries + 1}): {e}"
            )
        except Exception as e:
            logging.error(
                f"Unexpected error checking job {job_id} (Attempt {retries + 1}): {e}"
            )
        ...
        if retries <= config.ACTIVE_CHECK_MAX_RETRIES:
            ...
            logging.info(
                f"Retrying job {job_id} check after {wait_time:.2f} seconds..."
            )
            await asyncio.sleep(wait_time)

    logging.error(
        f"Failed to check job {job_id} activity after {config.ACTIVE_CHECK_MAX_RETRIES + 1} attempts."
    )
...
async def mark_expired_jobs():
    ...
    excluded_statuses = [
        ...
        "offer",
        "interviewing",
    ]  # Add any status that means "don't expire"
    ...
    try:
        ...
        if response.data:
            ...
            logging.info(
                f"Found {len(job_ids_to_expire)} jobs older than {config.JOB_EXPIRY_DAYS} days to mark as expired."
            )
            ...
            if job_ids_to_expire:
                ...
                if hasattr(update_response, "data") and update_response.data:
                    ...
                    )  # Supabase often returns the updated rows
                    logging.info(
                        f"Successfully marked {updated_count} jobs as expired."
                    )
                elif (
                    hasattr(update_response, "count")
                    and update_response.count is not None
                ):
                    logging.info(
                        f"Successfully marked {update_response.count} jobs as expired (based on count)."
                    )
                else:
                    # Log raw response if structure is unexpected
                    logging.warning(
                        f"Mark expired jobs update executed. Response: {update_response}"
                    )
...
async def check_linkedin_job_activity():
    ...
    try:
        ...
        if response.data:
            ...
            logging.info(
                f"Found {len(jobs_to_check)} active jobs to check (limit: {config.JOB_CHECK_LIMIT})."
            )
        else:
    ...
    logging.info(
        f"Activity Check Summary: Inactive={len(inactive_job_ids)}, Active={len(active_checked_job_ids)}, Failed={len(failed_check_job_ids)}"
    )
    ...
    try:
        if inactive_job_ids:
            ...
            logging.info(
                f"Marked {len(inactive_job_ids)} jobs as removed. Response: {update_inactive}"
            )
        ...
        if active_checked_job_ids:
            ...
            logging.info(
                f"Updated last_checked for {len(active_checked_job_ids)} active jobs."
            )
...
async def delete_old_inactive_jobs():
    ...
    try:
        ...
        if hasattr(delete_response, "data") and delete_response.data:
            ...
            )  # Delete often returns the deleted rows
        elif hasattr(delete_response, "count") and delete_response.count is not None:
            deleted_count = delete_response.count

        if deleted_count > 0:
            logging.info(
                f"Successfully deleted {deleted_count} inactive jobs older than {config.JOB_DELETION_DAYS} days."
            )
        else:
...
async def main():
    ...
    end_time = time.time()
    logging.info(
        f"Job Management Script finished in {end_time - start_time:.2f} seconds."
    )
